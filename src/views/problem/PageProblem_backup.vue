    ...
    <div class="problem-container">
        ...
        <div class="search-section">
            <div class="search-row">
                ...
                <!-- 标签选择 -->
                <el-button @click="drawer = true; fetchTags()" size="small"
                    :type="problemQueryDTO.tagId.length > 0 ? 'primary' : ''" plain>
                ...
                <!-- 添加题目（如果有权限） -->
                <el-button v-if="userInfo.permissions.includes('problem:add')" size="small" type="success"
                    icon="el-icon-plus" @click="jump(`/problem/add`)">
                    添加题目
                </el-button>
            </div>
            ...
            <div class="selected-tags" v-if="problemQueryDTO.tagId.length > 0">
                ...
                <el-tag v-for="tagId in problemQueryDTO.tagId" :key="tagId" closable @close="removeTag(tagId)"
                    size="small" class="selected-tag">
                    {{ getTagName(tagId) }}
                </el-tag>
            ...
            <!-- 标签抽屉 -->
            <el-drawer title="选择标签" :visible.sync="drawer" direction="rtl" size="360px">
        ...
        <div class="table-section">
            ...
            <div class="stats-bar" v-if="!loading">
                ...
                <div class="table-actions">
                    <el-tooltip content="刷新列表" placement="top">
                        <el-button type="text" icon="el-icon-refresh" @click="refreshProblems" :loading="loading"
                            size="small">
                        </el-button>
                    </el-tooltip>
                </div>
            ...
            <!-- 题目表格 -->
            <el-table :data="problems" style="width: 100%" v-loading="loading" element-loading-text="加载中..."
                size="small" stripe highlight-current-row @row-click="handleRowClick" class="problem-table">
                ...
                <el-table-column prop="tags" label="标签" width="160" v-if="showTags">
                    <template v-slot="scope">
                        <div class="tags-container">
                            <el-tag v-for="(tag, index) in scope.row.tags.slice(0, 2)" :key="index" size="mini"
                                effect="plain" class="problem-tag">
                                {{ tag }}
                            </el-tag>
            ...
            <div class="pagination-wrapper">
                <el-pagination @current-change="handleCurrentChange" @size-change="sizeChange"
                    :current-page="problemQueryDTO.page" :page-sizes="[10, 20, 50, 100]"
                    :page-size="problemQueryDTO.pageSize" layout="total, sizes, prev, pager, next, jumper"
                    :total="problemQueryDTO.total" :disabled="loading" background small>
                </el-pagination>
            </div>
        </div>
    </div>
